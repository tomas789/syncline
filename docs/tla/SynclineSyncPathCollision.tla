...
ClientApplyRemote(c) ==
    ...
    /\ LET msg == Head(serverToClient[c])
       ...
       IN
       CASE msg.type = MsgS2 ->
            \* Find colliding docs: local freshly-created docs at same path,
            \* where the incoming doc was on server when we connected
            LET colliders == {d2 \in DocIds :
                  /\ d2 /= d
                  /\ clientDiskDocAt[c][d2] = incomingPath
                  /\ incomingPath /= NullPath
                  /\ d2 \in freshlyCreated[c]
                  /\ d \in initialServerDocs[c]}
            IN
            IF colliders /= {}
            THEN
            ...
            ELSE
              ...
              /\ clientHasDoc'    = [clientHasDoc EXCEPT ![c][d] = TRUE]
              /\ clientDocPath'   = [clientDocPath EXCEPT ![c][d] = incomingPath]
              /\ clientDiskDocAt' = [clientDiskDocAt EXCEPT ![c][d] = incomingPath]
              /\ clientSubscribed' = [clientSubscribed EXCEPT ![c] = @ \cup {d}]
